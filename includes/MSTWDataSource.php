<?php
/*--------------------------------------------------------------------------
 *  MSTWDataSource - a few tools for csv data file import into MSTW plugins
 *
 *  Modified from DataSource.php by Kazuyoshi Tlacaelel <kazu.dev@gmail.com>
 *	Copyright (c) <2008> <Kazuyoshi Tlacaelel> under the MIT Licence
 *
...
if ( !class_exists( 'MSTW_CSV_DataSource' ) ) {
	class MSTW_CSV_DataSource {
		 ...
		 * all non-existent cells will be filled with empty spaces
		 *
		...
		protected function parse()
		{
			...
			while ($keys = fgetcsv($res, $l, $d, $e)) {
				//This ignores a blank first line in csv file
				if( $keys[0] != null) {
					if ($c == 0) {
						$this->headers = $keys;
					} else {
	...
	} //End: class MSTW_CSV_DataSource
} //End: if (!class_exists( MSTW_CSV_DataSource )
